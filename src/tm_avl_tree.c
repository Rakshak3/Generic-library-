...
//------(Postorder Iterator starts here)
...
if(avlTree->start==NULL) return avlTreePostOrderIterator;
...
while(t!=NULL)
{
if(t->right) pushOnStack(avlTreePostOrderIterator.stack,(void *)t->right,&succ);
if(succ==false)
{
...
{
(AVLTreeNode *)popFromStack(avlTreePostOrderIterator.stack,&succ);
pushOnStack(avlTreePostOrderIterator.stack,(void *)t,&succ);
t=t->right;
}
else
...
{
AVLTreeNode **p2p;
AVLTreeNode *root,*lc,*rc,*lcrc,*rclc,*parent;
int lh,rh,diff,succ;
...
{
rc=root->right;
if(getHeightOfAVLTree(root->left)>getHeightOfAVLTree(root->right))
{
rclc=rc->left;
rc->left=rclc->right;
rclc->right=rc;
...
{
lc=root->left;
if(getHeightOfAVLTree(root->right)>getHeightOfAVLTree(root->left))
{
lcrc=lc->right;
lc->right=lcrc->left;
lcrc->left=lc;
...
printf("yes call end successfuly\n");
}
}
